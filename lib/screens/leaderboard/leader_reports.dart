...
class LeaderReports extends StatefulWidget {
  final String id;
  final String name;
  final String regNo;
  ...
  const LeaderReports(
      {Key key, this.id, this.name, this.regNo, this.company, this.contact})
      : super(key: key);

  // final String title;

  @override
  State<LeaderReports> createState() => _LeaderReportsState();
}

class _LeaderReportsState extends State<LeaderReports> {
  int selectedVal;
  ...
  getCompany() async {
    try {
      await APIs().getMyCompany(userId: widget.id).then((value) {
  ...
  getAnnualPlan() async {
    try {
      savedPlans.clear();
      await APIs()
          .getAnnualPlans(userId: widget.id, year: _chosenValue)
          .then((value) {
  ...
  getAnnualReport() async {
    try {
      savedPlans.clear();
      await APIs()
          .getAnnualReport(userId: widget.id, year: _chosenValue)
          .then((value) {
        if (value.done) {
